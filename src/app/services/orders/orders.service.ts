...
export class OrdersService {
  ...
  private _loggedWaiter: Waiter;
  private _loggedWaiter2: Waiter;
  ...
  constructor(private _httpClient: HttpClient, private _dishService: DishesService) {
    ...
    this._loggedWaiter = new Waiter(this.getRandomId().toString(),'Pawel','Pawełski');
    this._loggedWaiter2 = new Waiter(this.getRandomId().toString(),'Adam','Adamski');
    ...
    let tmpOrder = new Order('1',  this._loggedWaiter2, 5, new Date(this._currentDate.getTime() - (1000 * 60 * 15))
    ...
    tmpOrder = new Order('2',  this._loggedWaiter2, 4, new Date(this._currentDate.getTime() - (1000 * 60 * 10))
    ...
    for (let i = 1; i < 10; i++) {
      for (let y = 8; y <= 22; y++) {
        ...
        for (let z = 0; z < 10; z++) {
          ...
          let tmpOrder = new Order(this.getRandomId().toString(),  this._loggedWaiter, 5, date
